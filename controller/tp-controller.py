...
# Function to reset all registers on all switches
def reset_all_registers(connections, program_config):
    
    for sw_name, sw in connections.items():
        helper = program_config[sw_name]["helper"]
        # Get registers through the p4info file
        for reg in helper.p4info.registers:
            reg_id = reg.preamble.id
            reg_size = reg.size
            wr.reset_register(helper, sw, reg_id, reg_size)
...
def handle_reset(target, switches_config_path, switch_programs_path, tunnels_config_path, 
    ...
    if target == "all":
        ...
        # Reset all registers on all switches
        reset_all_registers(connections, program_config)
